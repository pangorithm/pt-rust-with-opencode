//! Rust `tokio-tungstenite`를 이용한 간단한 WebSocket 에코 서버 예제입니다.
//! 이 예제는 JavaScript 개발자가 익숙한 WebSocket 모델을 Rust의 비동기 Stream(Stream)/ 및 싱크(Sink) 모델로 어떻게 구현하는지 이해하는 것을 목표로 합니다.

use futures_util::{
    sink::SinkExt,     // Stream에서 메시지를 보내기 위한 send() 메서 위한 트레이트
...
/// 클라이언트와의 WebSocket 통신을 담당하는 함수입니다.
async fn handle_connection(raw_stream: TcpStream, addr: SocketAddr) {
    ...
    println!("✅ 핸드셰이크 완료: {}", addr);

    // 5. WebSocket 스트림 분리
    // `split()`을 호출하면 읽기(`Stream`)와 쓰기(`Sink`)를 분리할 수 있습니다.
    // 이는 JavaScript에서 `onmessage` 이벤트와 `socket.send()`를 따로 사용하는 것과 유사합니다.
    let (mut write, mut read) = ws_stream.split();

    // 6. 메시지 수신 루프
    // `read.next()`는 클라이언트로부터 메시지가 올 때까지 비동기적으로 대기합니다.
    // 이는 JS의 `socket.onmessage`와 개념적으로 유사하지만, 지속적인 비동기 스트림 방식입니다.
    ...
    println!("🔌 클라이언트({}) 연결이 해제되었습니다.", addr);
}
